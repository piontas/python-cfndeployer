...
# modified by piatek.marek@gmail.com
...
from cfndeployer.exceptions import ExportException, \
    InvalidTemplateUrlParameterError, InvalidTemplatePathError
...
def parse_s3_url(url,
    ...
    if isinstance(url, string_types) and url.startswith("s3://"):
...
def upload_local_artifacts(resource_id, resource_dict, property_name,
    ...
    raise InvalidTemplatePathError(resource_id=resource_id,
...
class CloudFormationStackResource(Resource):
    ...
    def do_export(self, resource_id, resource_dict, parent_dir):
        ...
        if template_path is None or is_s3_url(template_path) or \
                template_path.startswith("https://s3.amazonaws.com/") or \
                template_path.startswith("https://s3-"):
            # Nothing to do
            return
